...
public class Agenda extends AgendaUtil {
    ...
    public void Excluir(Contatos con) {
        slv_text = "DELETE FROM CONTATOS"
                + " WHERE NOME = '" + con.getNome().toUpperCase() + "'"
                + " AND ENDERECO = '" + con.getEndereco().toUpperCase() + "'"
                + " AND EMAIL = '" + con.getEmail().toUpperCase() + "'"
                + " AND DDDRES = " + con.getTelres().split("#")[0]
                + " AND TELRES = " + con.getTelres().split("#")[1]
                + " AND DDDCOM = " + con.getTelcom().split("#")[0]
                + " AND TELCOM = " + con.getTelcom().split("#")[1];
        cdb.Inserir(slv_text);
    }
    ...
    private int Indice(String nome, Contato c) throws SQLException, NumberFormatException, Exception {
        ...
        rs = cdb.Executar("SELECT INDICE, NOME, ENDERECO, EMAIL, DDDRES, TELRES, DDDCOM, TELCOM FROM CONTATOS WHERE NOME LIKE '%" + nome.toUpperCase() + "%' ORDER BY INDICE ASC");
        if (null != rs) {
            while (rs.next()) {
                aux = rs.getInt(1);
